...
![运行截图](screenshots/maa_auto.png)

本项目的主要使用情景是：你的控制端安装在Linux上，你希望能够同时控制多个容器中的MAA/BAAH或者其他终端程序按照自己希望的时段执行预设任务，并且希望通过web面板控制。本工具只当作调度程序，即在什么时间段执行哪些任务，并在达成条件（运行成功、运行失败、公招出六星）后进行通知提醒，对于具体的任务荷载仍然需要自行设置。
...
本仓库提供的调度脚本仅用于技术学习，不代表也不鼓励任何对游戏、软件或服务条款的违规使用。使用者应自行确保任务配置与运行方式符合所在地区法律法规以及相关游戏/服务的用户协议。作者及贡献者不对由此带来的账号封禁、数据损坏或其他损失承担责任。如有疑虑，请立即停止使用并联系相关厂商确认。
