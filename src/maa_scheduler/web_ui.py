...
# 任务管理
@app.get("/api/tasks", response_model=List[TaskConfig])
async def get_tasks():
    """获取所有任务的配置"""
...
async def run_task_manually(task_id: str):
    ...
    try:
        ...
        # 手动执行任务，绕过调度器
        execution_id = await task_executor.execute_task(task)
        return {"message": "任务已开始手动执行", "execution_id": execution_id}
    except Exception as e:
...
async def save_app_config(config: AppConfig):
    """保存应用配置"""
    try:
        # 从传入的配置中排除任务列表，因为任务是单独管理的
        config_to_save = config.copy(exclude={'tasks'})
        
        # 获取当前的任务列表并设置回去，以防丢失
        current_tasks = config_manager.get_config().tasks
        config_to_save.tasks = current_tasks

        config_manager.save_config(config_to_save)
        
        # 可能需要重新加载某些服务
        await scheduler.reload_tasks() # 任务相关的配置也可能在AppConfig中
